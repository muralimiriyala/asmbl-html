...
jqr5(function () {
  if (jqr5('#resources-load-more').length) {
    ...
    i.append(
      '<div id="load-more" class="load-more-resources flex flex-vcenter"><span class="loader"></span><div class="text button button-lg"><span>' +
        jqr5button +
        '</span></div></div>',
    )
    var u = function () {
    ...
    if (postsCount <= 9) {
      ...
      if (postsCount == 0) {
        jQuery('.resource-blog-main').append(
          "<h3 class='no_result_found'>Sorry, No posts found.</h3>",
        )
      }
    }
